...
class BaseFederatedTrainer(object):
    def __init__(
        ...
        **kwargs,
        ...
        # self.grad_norm_list = [[] for i in range(self.client_num)]
        (
            self.train_loss,
            self.train_acc,
            self.val_loss,
    ...
    def train_epoch(self, a_iter, train_loaders, loss_fun, datasets):
        for client_idx, model in enumerate(self.client_models):
            ...
            # self.grad_norm_list[client_idx].append(torch.norm(flatten(client_update)).item())
        ...
        for a_iter in range(self.start_iter, self.args.rounds):
            ...
            with torch.no_grad():
                try:
                    ...
                    # self.server_model, self.client_models = self.communication(
                    #     self.server_model, self.client_models, self.client_weights
                    # )
